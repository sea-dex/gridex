...
contract Linear is IGridStrategy {
    ...
    function createGridStrategy(bool isAsk, uint128 gridId, bytes memory data) external override {
    ...
    function validateParams(bool isAsk, uint128 amt, bytes calldata data, uint32 count) external pure override {
        ...
        if (isAsk) {
            ...
            require(uint256(price0) + uint256(count - 1) * uint256(int256(gap)) < uint256(type(uint256).max), "L4");
            require(FullMath.mulDivRoundingUp(uint256(amt), uint256(price0 - uint256(gap)), PRICE_MULTIPLIER) > 0, "Q0");
        } else {
            require(gap < 0, "L5");
            require(uint256(price0) + uint256(-int256(gap)) < uint256(type(uint256).max), "L6");
            int256 priceLast = int256(uint256(price0)) + int256(gap) * int256(int32(count) - 1);
            require(priceLast > 0, "L7");
            require(FullMath.mulDivRoundingUp(uint256(amt), uint256(priceLast), PRICE_MULTIPLIER) > 0, "Q1");
        }
    }
