...
contract DeployDeterministic is Script {
    ...
    function run() public {
        ...
        // Step 1: Deploy Vault (same address on all chains - no constructor args)
        ...
        // Step 2: Deploy GridEx
        // NOTE: Address will differ per chain if WETH/USD addresses differ
        deployedGridEx = _deployGridEx(weth, usd, deployedVault);
        require(deployedGridEx == expectedGridEx, "GridEx address mismatch");
        console.log("[OK] GridEx deployed at:", deployedGridEx);
        
        // Step 3: Deploy Linear Strategy
        ...
        // Step 4: Whitelist Linear strategy in GridEx
    ...
    function deployGridExOnly() public {
        ...
        deployedGridEx = _deployGridEx(weth, usd, vault);
    ...
    function _deployGridEx(address _weth, address _usd, address _vault) internal returns (address) {
        bytes memory bytecode = abi.encodePacked(
            type(GridEx).creationCode,
            abi.encode(_weth, _usd, _vault)
        );
    ...
    function previewAddresses(address _weth, address _usd) public pure returns (
        ...
        // Compute GridEx address
        bytes memory gridExBytecode = abi.encodePacked(
            type(GridEx).creationCode,
            abi.encode(_weth, _usd, expectedVault)
        );
