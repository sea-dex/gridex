...
contract GridExFillETHQuoteTest is GridExBaseTest {
    ...
    function test_fillETHQuoteAskOrder() public {
        ...
        // eth balance
        assertEq(0, eth.balanceOf(address(exchange)));
        assertEq(calcMakerFee(fee) + (amt * gap) / PRICE_MULTIPLIER, gridConf.profits);
        assertEq(
            initialETHAmt * 2,
        ...
        assertEq(ethVol + fee, weth.balanceOf(address(exchange)) + weth.balanceOf(vault));
        assertEq(initialETHAmt - ethVol - fee, eth.balanceOf(taker));

        // assertEq(exchange.protocolProfits(Currency.wrap(address(weth))), fee >> 1);

        // grid profit
        assertEq(gridConf.profits, calcMakerFee(fee) + (amt * gap) / PRICE_MULTIPLIER);
        ...
        (uint128 ethVol2, uint128 fees2) = Lens.calcBidOrderQuoteAmount(askPrice0 - gap, amt, 500);
        gridConf = exchange.getGridConfig(1);
        // grid profit
        assertEq(
            gridConf.profits,
        ...
        // taker balance
        assertEq(initialETHAmt - ethVol - fee + ethVol2 - fees2, eth.balanceOf(taker) + weth.balanceOf(taker));
    ...
    function test_partialFillETHQuoteAskOrders() public {
        ...
        (uint128 ethVol0, uint128 fee0) = Lens.calcAskOrderQuoteAmount(askPrice0, amt, 500);
        (uint128 ethVol1, uint128 fee1) = Lens.calcAskOrderQuoteAmount(askPrice0 + gap, amt, 500);
        (uint128 ethVol2, uint128 fee2) = Lens.calcAskOrderQuoteAmount(askPrice0 + gap * 2, amt / 4, 500);
        uint128 ethVolTotal = ethVol0 + ethVol1 + ethVol2;
        uint128 feeTotal = fee0 + fee1 + fee2;

        vm.startPrank(taker);
        exchange.fillAskOrders{value: ethVolTotal + feeTotal}(1, orderIds, amts, amt * 3, 0, new bytes(0), 1); // inToken: ETH
        vm.stopPrank();
        ...
        assertEq(initialSEAAmt * 2, sea.balanceOf(maker) + sea.balanceOf(taker) + sea.balanceOf(address(exchange)));
        // grid order flipped
        IGridOrder.OrderInfo memory order0 = exchange.getGridOrder(toGridOrderId(1, orderId));
        IGridOrder.OrderInfo memory order1 = exchange.getGridOrder(toGridOrderId(1, orderId + 1));
        IGridOrder.OrderInfo memory order2 = exchange.getGridOrder(toGridOrderId(1, orderId + 2));
        IGridOrder.GridConfig memory gridConf = exchange.getGridConfig(1);
        ...
        (uint128 vol0, uint128 revVol0, uint128 profit0,) = calcQuoteVolReversed(price0, gap, amts[0], amt, 0, 500);
        (uint128 vol1, uint128 revVol1, uint128 profit1,) = calcQuoteVolReversed(price1, gap, amts[1], amt, 0, 500);
        (uint128 vol2, uint128 revVol2, uint128 profit2,) = calcQuoteVolReversed(price2, gap, amtFilled3, amt, 0, 500);
        ...
        assertEq(initialETHAmt - vol0 - vol1 - vol2 - fees, eth.balanceOf(taker));
        assertEq(vol0 + vol1 + vol2 + fees, weth.balanceOf(address(exchange)) + weth.balanceOf(vault));
    ...
    function test_fillETHQuoteBidOrder() public {
        ...
        // eth balance
        (uint128 ethVol, uint128 fees) = Lens.calcBidOrderQuoteAmount(bidPrice0, amt, 500);
        assertEq(initialETHAmt + ethVol - fees, eth.balanceOf(taker));
        assertEq(initialETHAmt - quoteAmt, eth.balanceOf(maker));
        assertEq(quoteAmt - ethVol + fees, weth.balanceOf(address(exchange)) + weth.balanceOf(vault));
        // assertEq(
        //     exchange.protocolProfits(Currency.wrap(address(weth))),
        //     fees >> 1
        // );
        ...
        (uint128 ethVol2, uint128 fees2) = Lens.calcAskOrderQuoteAmount(bidPrice0 + gap, amt, 500);
        // fill reversed order
        vm.startPrank(taker);
        ...
        assertEq(initialSEAAmt * 2, sea.balanceOf(maker) + sea.balanceOf(taker) + sea.balanceOf(address(exchange)));
        assertEq(initialSEAAmt, sea.balanceOf(taker));
        assertEq(initialETHAmt - ethVol2 - fees2 + ethVol - fees, eth.balanceOf(taker));
        assertEq(
            initialETHAmt * 2,
            eth.balanceOf(maker) + eth.balanceOf(taker) + weth.balanceOf(address(exchange)) + weth.balanceOf(vault)
        );
        ...
        // taker balance
        assertEq(initialETHAmt + ethVol - fees - ethVol2 - fees2, eth.balanceOf(taker));
        assertEq(initialSEAAmt, sea.balanceOf(taker));
    }
    ...
    function test_partialFillETHQuoteBidOrders() public {
        ...
        assertEq(initialETHAmt + fillVol0 - fee0 + fillVol1 - fee1 + fillVol2 - fee2, eth.balanceOf(taker));
