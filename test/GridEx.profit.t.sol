...
contract GridExProfitTest is GridExBaseTest {
    ...
    function test_profitAskOrder() public {
        ...
        vm.startPrank(taker);
        exchange.fillAskOrder{value: ethVol+fees}(orderId, amt, amt, 1);
        vm.stopPrank();
        ...
        vm.startPrank(third);
        vm.expectRevert();
        exchange.collectProtocolFee(Currency.wrap(address(weth)), third, fees/4, 0);
        vm.stopPrank();
        ...
        assertEq(initialETHAmt + gapProfit + fees - (fees>>1) - fees/4, eth.balanceOf(maker));
    }
}
